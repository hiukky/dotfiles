...
`run_once_before_16-ollama-windows.sh` is also WSL-specific, for the same shell-out-to-`powershell.exe` reason: Ollama needs to run natively on Windows to access the GPU (WSL's own GPU passthrough is not what you want for a long-running local inference server), so this installs it on the Windows host via its official Inno Setup installer run with `/VERYSILENT /NORESTART`. Like the font script, its effects are host-wide, not per-distro.
...
**Short URLs (`get.hiukky.com`, `vercel.json`):** `README.md`, `setup.ps1`, and `setup.sh` all reference `https://get.hiukky.com/setup.sh` and `.../setup.ps1` instead of the long `raw.githubusercontent.com` URLs. `vercel.json` (repo root, `.chezmoiignore`d, same as `setup.*`) defines `rewrites` that proxy those two paths straight to the corresponding `raw.githubusercontent.com` files, so the client never sees a redirect, it just gets the file content directly. `get.hiukky.com` is a Vercel project deployed from this same repo; the root domain `hiukky.com` already hosts something else, so this lives on a dedicated subdomain rather than risking the main domain's config. `hiukky.com`'s DNS is managed by Vercel (`ns1`/`ns2.vercel-dns.com`), with a legacy Route53 zone also still resolvable for it, a leftover from a prior DNS provider; if `get.hiukky.com` ever needs touching again, check both DNS providers before assuming propagation delay, since a stale Route53 record silently shadowing the Vercel one is a real thing that happened here.
...
- The install scripts under `.chezmoiscripts/` run automatically and unattended during `chezmoi apply`; only pipe `curl | sh`/`curl | bash` from a tool's own official install domain (as the existing scripts do: `get.chezmoi.io`, `starship.rs`, `mise.run`, `get.docker.com`, `claude.ai`, `herdr.dev`, plus binary/installer downloads straight from `dl.k8s.io`/`kind.sigs.k8s.io`/`ollama.com`/GitHub releases for `kubectl`/`kind`/`nvim`/Ollama). Don't add a new one without being sure of the source.
