...
`~/.local/bin/nerd-font-lab` (`dot_local/bin/executable_nerd-font-lab.tmpl`) reads the same catalog: `--list` shows which faces are installed on Windows, and `--specimen` draws uze's glyphs in the current terminal. Fonts are compared by setting one on the Ubuntu profile in Windows Terminal and running `--specimen`; the catalog adds no profiles of its own. The Windows Terminal default face is the Mono build of Fira Code: Mono is the build whose icons sit inside one cell, at a letter's size.
