...
Dotfiles alone are not enough: `.zshrc` assumes `zinit`, `eza`, `bat`, `fd`, `ripgrep`, `starship`, `mise`, `docker`, `claude`, `uze` etc. already exist. These are installed by scripts under `.chezmoiscripts/`, a directory chezmoi treats specially: its contents are executed as scripts (per the usual `run_once_`/`run_onchange_`/`before_`/`after_` naming rules), not written to `$HOME`, and are kept out of `chezmoi managed`'s regular dotfile listing conceptually (organizational separation from the actual dotfiles at the repo root).
...
`run_onchange_before_15-nerd-fonts.sh.tmpl` installs the Nerd Font catalog in `.chezmoidata/nerd_fonts.yaml` (one pinned release; per family an archive, its sha256 from the release's `SHA-256.txt`, and the plain and Mono faces) into the **Windows** user font store, and nowhere else. Under WSL the terminal is Windows Terminal, a Windows process that renders with Windows' fonts, so a Nerd Font installed on the Linux side is never drawn; the Linux side only downloads, verifies and extracts into `~/.cache/nerd-fonts/<release>`. It no-ops outside WSL, and its effects are host-wide rather than per distro. It is `run_onchange_` because the catalog is rendered into it: editing the catalog re-runs it. A font Windows has loaded cannot be overwritten, so an identical file is skipped and a changed one in use is reported and kept.

The same catalog renders one hidden `Nerd Lab · <face>` profile per face into `.chezmoitemplates/windows-terminal-settings.json`, and `~/.local/bin/nerd-font-lab` (`dot_local/bin/executable_nerd-font-lab.tmpl`) opens a Windows Terminal window with one tab per profile, each drawing uze's glyphs (`--specimen`), so fonts are compared side by side with nothing else differing; `--list` shows which are installed on Windows. The Windows Terminal default face is the Mono build of Fira Code: Mono is the build whose icons sit inside one cell, at a letter's size.
...
`run_onchange_before_07-idle-guard.sh.tmpl` is the third file in this cluster, and the only one that also changes Windows settings rather than just writing a file. It is the sleep profile for a machine that used to run 24/7: `powercfg /change` sets the AC idle timeouts (monitor 10 min, standby 30 min, hibernate off), and a per-user scheduled task runs `.chezmoitemplates/idle-guard.ps1`, which polls the Linux side once a minute and holds `ES_SYSTEM_REQUIRED` while WSL reports work.

The split exists because Windows' idle timer only counts keyboard and mouse input: on its own it would suspend the machine mid-build. Windows still owns the timeout and the suspend; the guard can only ever say "not yet". The Linux half is `dot_local/bin/executable_idle-guard` (`check`, `status`, `inhibit`, `release`, `report`) with its tunables in `dot_config/idle-guard/config`, re-read on every check so edits need no restart.

Design choices that are not obvious from the code:

- **Fails open, always.** If the guard dies, the distro is shut down, or the check errors, the answer is "idle" and the machine sleeps normally. A guard that can wedge a desktop permanently awake is worse than no guard, and it would fail the exact way it is meant to prevent: silently.
- **Presence, not CPU, for the work patterns.** An agent blocked on an API response or a build blocked on a network fetch burns ~0% CPU and must still hold the machine awake. The CPU threshold is only the catch-all for whatever the pattern list misses.
- **The pattern list is deliberately narrow and job-shaped** (`cargo build`, not bare `cargo`). This machine permanently runs `uze` daemons, jaeger, MCP servers and docker-proxy; a broad list matches those and the machine never sleeps again.
- **CPU comes from a `/proc/stat` delta, never load average.** Under WSL2 the load average reads far above actual utilisation -- measured here at 1.8 against a real 8% -- and on its own would veto sleep forever.
- **`/dev/pts` mtime was tried as an activity signal and rejected.** It looks ideal (an agent streaming output touches it, an idle shell does not), but tmux's status line redraws on its client's pts every few seconds, so an attached tmux session keeps it permanently hot.
- **The Windows side owns the log** (`%LOCALAPPDATA%\idle-guard\guard.log`), because it is the only half still running while the machine is asleep. It logs transitions, not every poll, plus a 30-minute heartbeat; a resume is inferred from a wall-clock gap much larger than the poll interval, which is the only way the guard can observe that it actually worked. `idle-guard report` reads that log back through `/mnt/c`.

Four gotchas, all found by running it rather than reading docs:

- **`Select-Object -First 1` on a native command destroys its exit code.** Stopping the pipeline early terminates the process and leaves `$LASTEXITCODE` at `-1`. Since the exit code *is* the busy/idle answer, the first working version silently decided "idle" forever and logged nothing, because the initial state was also idle. The output is collected into an array and indexed instead, and `$held` starts at `$null` so the very first decision is always logged.
- **Windows PowerShell 5.1 parses `0x80000000` as a signed `Int32`** and then throws converting the negative result to the `UInt32` that `SetThreadExecutionState` takes. `ES_CONTINUOUS` is written in decimal (`2147483648`) for that reason. Note also that `ES_CONTINUOUS` *alone* means "release the veto", not "no change".
- **`wsl.exe --list` emits UTF-16LE.** Read under the default console encoding the distro name comes back as `U\0b\0u\0n\0t\0u\0` and never matches. `[Console]::OutputEncoding` is set to `Unicode` for that call and back to `UTF8` for the `check` call, which passes through the Linux program's plain UTF-8.
- **The running distro is checked before asking it anything.** `wsl.exe -d <distro> -- ...` *starts* a stopped distro, so polling without the `--list --running` guard would boot the VM every 60 seconds and burn more power than the whole profile saves.

Task registration is non-admin (`Register-ScheduledTask` in the user's own namespace, no stored credentials), which has one visible cost: it runs in the interactive session, so `powershell.exe` flashes a window for an instant at logon. `ExecutionTimeLimit` must be `[TimeSpan]::Zero` -- the default three-day limit would quietly kill an infinite polling loop mid-week -- and `MultipleInstances IgnoreNew` stops a re-logon from stacking a second guard.

Hibernate-after-sleep is deliberately left at 0. S3 already takes this machine from roughly 80 W to 4 W, and S4 adds a slower resume plus more ways for a WSL2 VM to come back unhappy. Worth revisiting once the S3 cycle has proved itself; `powercfg /a` reports S3 and hibernate available here but **not** hybrid sleep, which Hyper-V (and therefore WSL2) disables.

`run_once_after_81-ccstatusline.sh` installs the `ccstatusline` npm package (Claude Code's HUD/statusline renderer) via `bun install -g`, resolving `bun` the same `command -v || fallback path` way as `mise`/`cargo`/`claude` elsewhere in this file — it must run `after` (numbered past `80-mise-install.sh`), since `bun` itself comes from `mise install` in that script. `dot_claude/settings.json` wires it up (`statusLine.command = "ccstatusline"`) and also carries the model default, enabled plugins, and TUI/theme prefs; `dot_config/ccstatusline/settings.json` is the HUD's own layout (which segments show, colors, separators) and is unrelated to `~/.claude/settings.json` beyond that one command reference.
...
`run_once_after_96-uze.sh` installs [uze](https://uze.hiukky.com) (agent-tooling compatibility layer plus terminal workspace; it replaced `herdr`, whose script and `dot_config/herdr/config.toml` were removed along with the tracked `neovim` config and `run_once_before_25-neovim.sh`). Unlike every other tool here it is **not** installed from its published installer (`curl -fsSL https://uze.hiukky.com/i | sh`): this machine develops uze, so the binary on `PATH` has to be the one built from the local checkout. The script clones `git@github.com:hiukky/uze.git` into `~/uze` over SSH only if that checkout doesn't exist yet, then `cargo install --path ~/uze --bin uze --locked --force` (landing in `~/.cargo/bin`, already on `PATH` via `dot_zshrc`).

Two constraints drive its numbering and its handling of an existing checkout:

- It is `after_96`, past `95-account-setup.sh`, because the SSH clone needs the machine's fresh key already registered with GitHub by that script. It also needs `cargo`, which comes from mise (`rust` in `dot_config/mise/config.toml`, installed by `80-mise-install.sh`), resolved via the usual `command -v cargo || ~/.local/share/mise/shims/cargo` fallback.
- `~/uze` on this machine is a live development checkout, not a build artifact. The script only runs `fetch`/`checkout main`/`merge --ff-only` when `git status --porcelain` is empty; with local changes present it builds the tree as-is and says so, rather than moving a branch out from under work in progress.
